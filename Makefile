...
### This is the Terraform-generated header for docker-geosolr-dev ###
ECR_NAME_DEV:=docker-geosolr-dev
ECR_URL_DEV:=222053980223.dkr.ecr.us-east-1.amazonaws.com/docker-geosolr-dev
...
# dist: ## Build docker container (intended for developer-based manual build)
# 	docker build --platform linux/amd64 \
# 	    -t $(ECR_URL_DEV):latest \
# 		-t $(ECR_URL_DEV):`git describe --always` \
# 		-t $(ECR_NAME_DEV):latest .

# publish: dist-dev ## Build, tag and push (intended for developer-based manual publish)
# 	docker login -u AWS -p $$(aws ecr get-login-password --region us-east-1) $(ECR_URL_DEV)
# 	docker push $(ECR_URL_DEV):latest
# 	docker push $(ECR_URL_DEV):`git describe --always`
